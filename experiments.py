    ...
    for l in f:
        ...
        if len(re.findall('\"(.+?)\"', l)):
            ...
            if start == 1:
                ...
                k = l.split(' ', 1)[0].decode('utf-8') + " " + (l.split(' ', 1)[1]).decode('utf-8')[
                                                               :start - temp].lower() + l[start:end].decode(
                    'utf-8') + l[end:].decode('utf-8').lower()
            print >> f2, k,
        else:
        ...
        if response['status'] == 'OK':
            for entity in response['entities']:
                if "type" in entity.keys:
                    if entity['type'] in ['Country', 'Holiday', 'Movie', 'MusicGroup', 'Organization', 'Person',
                                          'PrintMedia', 'Region', 'StateOrCountry', 'TelevisionShow',
                                          'TelevisionStation', 'Money', 'Company', 'GeographicFeature']:
...
def join_multiple_sentences(infile, outfile):
    ...
    while i < len(lines):
        ...
        if words_[0]['pos'] == ':':
            ...
            d["collapsed-ccprocessed-dependencies"] = sentence['collapsed-ccprocessed-dependencies'] + next_sentence[
...
def obtain_hyperbolic_terms(infile, outfile):
    ...
    for line in tagged_data:
        ...
        for word in sentence["tokens"]:
            ...
            try:
                if word[
                    'sentiment'] == u'Very negetive':  # Positive and word['pos'] in ['JJ', 'JJS', 'JJR', 'RB', 'RBS', 'RBR']:
