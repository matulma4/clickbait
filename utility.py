...
def convert_to_pattern(title):
    ...
    while i < len(tags):
        if tags[i] == '``':
            ...
            norm.append(words[i])
            i += 1
        elif tags[i] in ['CC', 'CD', 'DT', 'EX', 'IN', 'LS', 'MD', 'PDT', 'POS', 'PRP', 'PRP$', 'SYM', 'TO', 'UH',
                         'WDT', 'WP', 'WP$', 'WRB']:
            norm.append(words[i])
        else:
            norm.append(tags[i])
...
def findPaths(G, u, n, excludeSet=None):
    ...
    if n == 0:
        return [[u]]
    paths = [[u] + path for neighbor in G.neighbors(u) if neighbor not in excludeSet for path in
             findPaths(G, neighbor, n - 1, excludeSet)]
    excludeSet.remove(u)
    return paths
...
def naive_bayes(analysis):
    ...
    f1 = []
    current = list(nltk.ngrams(norm.split(), 1)) + list(nltk.ngrams(norm.split(), 2)) + list(
        nltk.ngrams(norm.split(), 3))
    ...
    # syntactic ngrams
    f3 = []
    current_sngrams = list(syntactic_n_gram(deps_cc, 1)) + list(syntactic_n_gram(deps_cc, 2)) + list(
        syntactic_n_gram(deps_cc, 3))
...
def create_vector(title):
    ...
    # subject of title
    common_subjects = ['dog', 'everyone', 'girl', 'girls', 'guy', 'guys', 'he', 'here', 'i', 'it', 'kid', 'kids', 'man',
                       'men', 'mom', 'one', 'parent', 'people', 'photos', 'reasons', 'she', 'signs', 'something',
                       'that', 'they', 'thing', 'things', 'this', 'thoughts', 'times', 'video', 'ways', 'we', 'what',
                       'who', 'woman', 'women', 'you']
    ...
    determiners = ['i', 'you', 'he', 'she', 'it', 'they', 'we', 'me', 'you', 'him', 'her', 'it', 'us', 'them', 'every',
                   'that', 'his', 'these', 'its', 'this', 'those', 'our', 'mine', 'ours', 'what', 'which', 'whose',
                   'your', 'his']
